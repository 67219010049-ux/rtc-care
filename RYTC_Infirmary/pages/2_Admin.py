...
# ==========================================
st.markdown(
    "<h1 class='responsive-h1'>แผงควบคุมสำหรับเจ้าหน้าที่ /"
    " อาจารย์ห้องพยาบาล</h1>",
...
# ==========================================
# หน้าฟอร์มล็อกอินเข้าสู่ระบบ
# ==========================================
if not st.session_state.admin_logged_in:
  col_l1, col_home_center, col_l2 = st.columns([1, 2, 1])
  with col_home_center:
    st.page_link(
        "Home.py", label="กลับหน้าหลัก (Home)", use_container_width=True
    )
  ...
  with st.container():
    st.markdown(
        ...
        " #2d3748;'>เข้าสู่ระบบเจ้าหน้าที่</h3>",
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    username = st.text_input(
        ...
        placeholder="พิมพ์ชื่อผู้ใช้งาน เช่น admin / ชื่ออาจารย์",
    )
    ...
    st.markdown(
        "<div style='padding-top: 10px;'></div>", unsafe_allow_html=True
    )
...
if st.session_state.admin_logged_in:
  ...
  with col_title:
    st.success(
        f"สถานะ: เจ้าหน้าที่ [{st.session_state.current_user}]"
        " เข้าสู่ระบบสำเร็จ"
    )
  with col_logout:
    if st.button(
        "ออกจากระบบ", use_container_width=True, key="logout_button"
    ):
      st.session_state.admin_logged_in = False
      st.session_state.current_user = ""
      st.rerun()
  ...
  if not os.path.exists(STOCK_FILE):
    pd.DataFrame([
        {
            "ชื่อยา/เวชภัณฑ์": "พาราเซตามอล (500mg)",
            "คงเหลือ": 300,
            "หน่วย": "เม็ด",
        },
    ...
    if os.path.exists(DB_FILE):
      ...
      for med in EXCLUDE_MEDICINES:
        if "การจ่ายยา/หมายเหตุ" in load_df.columns:
          ...
          load_df = load_df[
              ~load_df["การจ่ายยา/หมายเหตุ"]
              .astype(str)
              .str.contains(med, na=False)
          ]
      ...
      bad_cols = [
          c for c in load_df.columns if "暗" in c or c == "Unnamed: 0"
      ]
      ...
      if "สถานะการรักษา" in load_df.columns:
        load_df["สถานะการรักษา"] = (
            load_df["สถานะการรักษา"]
            .fillna("กำลังรอเจ้าหน้าที่คัดกรอง")
            .replace("รอดำเนินการคัดกรอง", "กำลังรอเจ้าหน้าที่คัดกรอง")
        )
      else:
        load_df["สถานะการรักษา"] = "กำลังรอเจ้าหน้าที่คัดกรอง"
  ...
  df_stock = df_stock[
      ~df_stock["ชื่อยา/เวชภัณฑ์"].isin(
          ["ยาขับเลือด", "ยาสตรี", "ยานอนหลับ"]
      )
  ]
    ...
    with m2:
      waiting_count = len(
          df[
              ~df["สถานะการรักษา"]
              .astype(str)
              .str.contains("เสร็จสิ้น|กลับไปเรียนต่อ|ส่งต่อ", na=False)
          ]
      )
      st.metric(
          "เคสที่ยังค้างอยู่ในระบบ (รอตรวจ/นอนพัก)", f"{waiting_count} เคส"
      )
    ...
    with tab2:
      try:
        ...
        df_time["datetime"] = pd.to_datetime(
            df_time["วันที่-เวลา"], errors="coerce"
        )
  ...
  with search_col1:
    search_id = st.text_input(
        "กรอกรหัสนักศึกษา 11 หลักเพื่อเช็กสถิติการรับยา:",
        placeholder="พิมพ์รหัส เช่น 67219010049",
    )
  if search_id.strip():
    ...
    if not student_history.empty:
      ...
      with search_col2:
        if total_records >= 4:
          custom_blue_box(
              "[!] คำเตือน:"
              " นักศึกษาคนนี้มารับบริการค่อนข้างบ่อยกว่าเกณฑ์ปกติ"
          )
        st.metric(
            label="จำนวนครั้งที่เข้าใช้บริการห้องพยาบาลรวม",
            value=f"{total_records} ครั้ง",
        )
      st.markdown(
          "##### ตารางประวัติของ: <span style='color:#001f3f;"
          f" font-weight:bold;'>{found_user_name}</span> [รหัส:"
          f" {search_id}]",
          unsafe_allow_html=True,
      )
      st.dataframe(
      ...
      with search_col2:
        custom_blue_box(
            "[!] ไม่พบประวัติเก่าในระบบ"
            " (นักศึกษาคนนี้เพิ่งเคยมาเป็นครั้งแรก)"
        )
  st.write("---")

  if df.empty:
    custom_blue_box("[!] ปัจจุบันยังไม่มีข้อมูลนักเรียนมาลงชื่อเข้ารับบริการ")
  else:
    st.subheader("รายชื่อนักเรียนคิวเข้ารับบริการทั้งหมด")
    st.dataframe(df, use_container_width=True)

    with st.expander(
        "แผงควบคุมเจ้าหน้าที่: ลบประวัตินักเรียนออกจากฐานข้อมูล"
    ):
      st.markdown(
          "<p style='color:#dc3545;'>อาจารย์สามารถเลือกลบรายชื่อนักเรียนที่คีย์ซ้ำ"
          " พิมพ์ข้อมูลผิด หรือต้องการตัดประวัติออกได้จากตรงนี้ครับ</p>",
          unsafe_allow_html=True,
      )
      delete_options = [
          f"{row['วันที่-เวลา']} | {row['ชื่อ-นามสกุล']}"
          f" ({row['รหัสนักศึกษา']}) - {row['อาการเบื้องต้น']}"
          for idx, row in df.iterrows()
      ]
      selected_student_to_delete = st.selectbox(
          "เลือกแถวประวัตินักเรียนที่ต้องการลบถาวร:",
          delete_options,
          key="admin_delete_student_selectbox",
      )
      del_time = selected_student_to_delete.split(" | ")[0]
      del_name = selected_student_to_delete.split(" | ")[1].split(" (")[0]

      admin_delete_confirm = st.button(
          "ยืนยันลบประวัตินักเรียนรายนี้ออกถาวร", use_container_width=True
      )
      if admin_delete_confirm:
        df = df[
            ~(
                (df["วันที่-เวลา"] == del_time)
                & (df["ชื่อ-นามสกุล"] == del_name)
            )
        ]
        save_data(df)
        st.success(
            f"ลบประวัติข้อมูลคิวของ {del_name} ออกจากระบบเรียบร้อยแล้ว"
        )
        time.sleep(2)
        st.rerun()
    ...
    if waiting_df.empty:
      ...
      waiting_options = [
          f"{row['ชื่อ-นามสกุล']} ({row['รหัสนักศึกษา']}) -"
          f" [{row['สถานะการรักษา']}]"
    ...
    if not waiting_options:
      ...
      selected_option = st.selectbox(
          "เลือกชื่อนักเรียนที่ต้องการวินิจฉัย / อัปเดตสถานะ:",
          waiting_options,
      )
      selected_student_name = selected_option.split(" (")[0]
      student_info = waiting_df[
          waiting_df["ชื่อ-นามสกุล"] == selected_student_name
      ].iloc[-1]

      # แจ้งเตือนเมื่อพบโรคประจำตัว (ใช้สัญลักษณ์ทางการ)
      disease_info = str(student_info.get("โรคประจำตัว", "-")).strip()
      if disease_info and disease_info not in ["-", "ไม่มี", "nan", "None"]:
        custom_red_box(
            f"[!] **แจ้งเตือนสำคัญ:** {selected_student_name} มีโรคประจำตัว:"
            f" **{disease_info}** (โปรดระมัดระวังในการสั่งจ่ายยา)"
        )

      custom_yellow_box(
          f"<b>เคสตรวจรักษาและสั่งยา:</b> {selected_student_name} |"
          f" <b>สถานะปัจจุบัน:</b> {student_info['สถานะการรักษา']} |"
          f" <b>อาการเบื้องต้น:</b> {student_info['อาการเบื้องต้น']}"
      )

      if "ให้นอนพักฟื้น" in str(student_info["สถานะการรักษา"]):
        st.info(
            " นักศึกษาคนนี้กำลังนอนพักฟื้นอยู่"
            " หากเขานอนพักเสร็จสิ้นแล้ว"
            " อาจารย์สามารถกดปุ่มปิดเคสด้านล่างเพื่อส่งกลับชั้นเรียนได้ทันที"
        )
        finish_rest_btn = st.button(
            " นักศึกษานอนพักเสร็จสิ้นแล้ว (ส่งกลับชั้นเรียน)",
            use_container_width=True,
        )
        if finish_rest_btn:
          ...
          send_telegram_notification(
              ...
              " นอนพักฟื้นเสร็จสิ้นและส่งกลับเข้าเรียนตามปกติ\nผู้บันทึก:"
              f" {st.session_state.current_user}"
          )
          st.success(
              f"อัปเดตสถานะของ {selected_student_name}"
              " เป็นเสร็จสิ้นเรียบร้อยแล้ว"
          )
          time.sleep(2)
          st.rerun()
        st.write("---")

      treatment_status = st.radio(
          "สั่งการรักษาทางการแพทย์:",
          [
              "จ่ายยาและให้กลับไปเรียนต่อ",
              "ให้นอนพักฟื้นที่ห้องพยาบาล",
              "ส่งต่อโรงพยาบาลภายนอก (เคสฉุกเฉิน)",
          ],
      )
      ...
      with med_col1:
        selected_med = st.selectbox(
            "เลือกตัวยาที่จ่ายให้เด็ก:", df_stock["ชื่อยา/เวชภัณฑ์"].tolist()
        )
      with med_col2:
        selected_qty = st.number_input(
            "จำนวนที่จ่าย (หน่วยตามชนิดยา):",
            min_value=0,
            max_value=50,
            value=2,
        )
      treatment_detail = st.text_input(
          "ระบุรายละเอียดตัวยาที่จ่าย / อาการเพิ่มเติม:"
      )

      update_btn = st.button("บันทึกและอัปเดตสถานะ", type="primary")
      if update_btn:
        stock_idx = df_stock[
            df_stock["ชื่อยา/เวชภัณฑ์"] == selected_med
        ].index[0]
        current_stock_qty = df_stock.at[stock_idx, "คงเหลือ"]
        if selected_qty > current_stock_qty:
          st.error(
              "ไม่สามารถบันทึกได้! ยาในคลังมีไม่พอจ่าย"
              f" (ในคลังเหลืออยู่เพียง {current_stock_qty} หน่วย)"
          )
        else:
          idx = df[df["ชื่อ-นามสกุล"] == selected_student_name].index[-1]
          df.at[idx, "สถานะการรักษา"] = treatment_status
          final_note = (
              f"จ่าย: {selected_med} ({selected_qty} หน่วย) | หมายเหตุ:"
              f" {treatment_detail}"
          )
          ...
          if selected_qty > 0:
            ...
            if df_stock.at[stock_idx, "คงเหลือ"] <= 20:
              send_telegram_notification(
                  "[แจ้งเตือนด่วน - RYTC คลังยา]\nเวชภัณฑ์:"
                  f" {selected_med}\nสถานะ: ใกล้หมดแล้ว! คงเหลือเพียง"
                  f" {df_stock.at[stock_idx, 'คงเหลือ']}"
                  f" {df_stock.at[stock_idx, 'หน่วย']}"
              )

          telegram_msg = (
              "[✅ รักษาสำเร็จ - RYTC Care]\n"
          ...
          st.success(
              "บันทึกผลการรักษาและหักยอดคลังยาของ"
              f" {selected_student_name} สำเร็จแล้ว"
          )
          time.sleep(2)
          st.rerun()
  ...
  for s_idx, s_row in df_stock.iterrows():
    with stock_cols[s_idx]:
      if s_row["คงเหลือ"] <= 20:
        custom_red_box(
            f"[!] **{s_row['ชื่อยา/เวชภัณฑ์']}**<br>ใกล้หมด! เหลือเพียง:"
            f" {s_row['คงเหลือ']} {s_row['หน่วย']}"
        )
      else:
        st.success(
            f"**{s_row['ชื่อยา/เวชภัณฑ์']}** \n\n คงเหลือ: {s_row['คงเหลือ']}"
            f" {s_row['หน่วย']}"
        )

  st.write("---")
  st.markdown(
      "### แผงบริหารและจัดการคลังยา (Inventory Management Control)"
  )
  stock_tab1, stock_tab2, stock_tab3 = st.tabs([
      "เติมยา / แก้ไขยอดคงเหลือด่วน",
      "ลงทะเบียนเพิ่มยาชนิดใหม่เข้าคลัง",
      "ลบรายชื่อยาและล้างประวัติขยะ",
  ])

  with stock_tab1:
    st.markdown("##### ระบบเติมยาเข้าคลัง หรือ พิมพ์ยอดแก้ไขสต็อกที่ผิดพลาด")
    add_col1, add_col2, add_col3, add_col4 = st.columns(
        [1.5, 1.2, 1.3, 1.2], gap="small"
    )
    with add_col1:
      restock_med = st.selectbox(
          "เลือกรายการยาที่ต้องการจัดการ:",
          df_stock["ชื่อยา/เวชภัณฑ์"].tolist(),
          key="restock_select",
      )
    with add_col2:
      manage_mode = st.radio(
          "เลือกการทำงาน:",
          ["เติมเพิ่มเข้าคลัง", "พิมพ์แก้ตัวเลขโดยตรง"],
          key="manage_mode_radio",
      )
    with add_col3:
      r_idx_check = df_stock[
          df_stock["ชื่อยา/เวชภัณฑ์"] == restock_med
      ].index[0]
      unit_label = df_stock.at[r_idx_check, "หน่วย"]
      if manage_mode == "เติมเพิ่มเข้าคลัง":
        input_qty = st.number_input(
            f"ระบุจำนวนที่เติมเพิ่ม ({unit_label}):",
            min_value=1,
            max_value=2000,
            value=100,
            key="restock_num",
        ...
        input_qty = st.number_input(
            f"ยอดคงเหลือที่ถูกต้อง ({unit_label}):",
            min_value=0,
            max_value=5000,
            value=int(df_stock.at[r_idx_check, "คงเหลือ"]),
            key="rewrite_num",
        )
    with add_col4:
      st.markdown(
      ...
      restock_btn = st.button(
          "ยืนยันบันทึกยอดคลังยา",
          type="primary",
          key="confirm_restock_btn",
          use_container_width=True,
      )
    if restock_btn:
      ...
      if manage_mode == "เติมเพิ่มเข้าคลัง":
        ...
        st.success(
            f"เพิ่มยอด {restock_med} สำเร็จจำนวน +{input_qty}"
            f" {df_stock.at[r_idx, 'หน่วย']} เรียบร้อยแล้ว"
        )
      else:
        df_stock.at[r_idx, "คงเหลือ"] = input_qty
        st.success(
            f"รีเซ็ตยอด {restock_med} เป็น {input_qty}"
            f" {df_stock.at[r_idx, 'หน่วย']} ตามที่แก้ไขเรียบร้อยแล้ว"
        )
      df_stock.to_csv(STOCK_FILE, index=False, encoding="utf-8-sig")
      time.sleep(2)
      st.rerun()
  ...
  with stock_tab2:
    ...
    new_col1, new_col2, new_col3, new_col4 = st.columns(
        [1.8, 1.0, 1.2, 1.2], gap="small"
    )
    with new_col1:
      new_med_name = st.text_input(
          "ชื่อยา/เวชภัณฑ์ใหม่ที่ต้องการเพิ่ม:",
          placeholder="เช่น ยาแก้ไอขับเสมหะ, ผ้าก๊อซพันแผล",
      )
    with new_col2:
      new_med_unit = st.text_input(
          "หน่วยนับ:", placeholder="เช่น เม็ด", value="เม็ด"
      )
    with new_col3:
      new_med_qty = st.number_input(
          "ยอดสต็อกเริ่มต้นที่มีอยู่:",
          min_value=0,
          max_value=5000,
          value=100,
      )
    with new_col4:
      ...
      add_new_med_btn = st.button(
          "ยืนยันลงทะเบียนยาใหม่",
          type="primary",
          key="add_new_med_btn",
          use_container_width=True,
      )
    if add_new_med_btn:
      if not new_med_name.strip():
        ...
        st.error(
            "ยาตัวนี้มีอยู่ในระบบแล้วครับ"
            " หากต้องการเพิ่มยอดให้ไปใช้แท็บแรก"
        )
      else:
        df_stock = pd.concat(
            [
                df_stock,
                pd.DataFrame([{
                    "ชื่อยา/เวชภัณฑ์": new_med_name.strip(),
                    "คงเหลือ": new_med_qty,
                    "หน่วย": new_med_unit.strip(),
                }]),
            ],
            ignore_index=True,
        )
        df_stock.to_csv(STOCK_FILE, index=False, encoding="utf-8-sig")
        st.success(
            f"สำเร็จ! ลงทะเบียน '{new_med_name.strip()}'"
            " เข้าสู่ระบบห้องพยาบาลเรียบร้อยแล้ว"
        )
        time.sleep(2)
        st.rerun()
  ...
  with stock_tab3:
    ...
    delete_med_target = st.selectbox(
        "เลือกชื่อยาที่ต้องการ ลบทำลายทิ้ง ออกจากฐานข้อมูลงาน:",
        df_stock["ชื่อยา/เวชภัณฑ์"].tolist(),
        key="delete_med_target_select",
    )
    st.markdown(
        "<p style='color:#dc3545; font-size:13px;'>หมายเหตุ: การกดปุ่มนี้จะลบตัวยาออกจากคลัง และกวาดล้างประวัติเก่าทั้งหมดที่มีชื่อยานี้อยู่ทิ้งทันที เพื่อไม่ให้มีคำนี้ติดไปในไฟล์รายงาน CSV ตอนส่งออกครับ</p>",
        unsafe_allow_html=True,
    )

    execute_delete_btn = st.button(
        "ยืนยันลบยาและล้างประวัติ",
        use_container_width=True,
        key="execute_delete_btn",
    )
    if execute_delete_btn:
      df_stock = df_stock[
          df_stock["ชื่อยา/เวชภัณฑ์"] != delete_med_target
      ]
      df_stock.to_csv(STOCK_FILE, index=False, encoding="utf-8-sig")
      if os.path.exists(DB_FILE):
        ...
        if "การจ่ายยา/หมายเหตุ" in raw_df.columns:
          raw_df = raw_df[
              ~raw_df["การจ่ายยา/หมายเหตุ"]
              .astype(str)
              .str.contains(delete_med_target, na=False)
          ]
        raw_df.to_csv(DB_FILE, index=False, encoding="utf-8-sig")
      st.success(
          f"ลบ '{delete_med_target}'"
          " และกวาดล้างข้อมูลขยะออกจากไฟล์ CSV เรียบร้อยแล้ว"
      )
      time.sleep(2)
      st.rerun()
  ...
  if not df.empty:
    ...
    st.download_button(
        ...
        file_name=(
            f"RTC_Care_Report_{datetime.now().strftime('%Y%m%d')}.csv"
        ),
        mime="text/csv",
