...
# 1. ตั้งค่าหน้าจอ
st.set_page_config(
    page_title="RYTC Infirmary", page_icon="✚", layout="centered"
)
...
# ==========================================
st.markdown(
    "<h1 class='responsive-h1'>ระบบลงทะเบียนผู้เข้ารับบริการห้องพยาบาล</h1>",
    unsafe_allow_html=True,
)
st.markdown(
...
st.info(
    "ประกาศประชาสัมพันธ์: ช่วงนี้อากาศเปลี่ยนแปลงบ่อย หากมีอาการไข้หรือไอ"
    " ขอความร่วมมือนักศึกษาสวมหน้ากากอนามัยก่อนเข้าใช้บริการห้องพยาบาลทุกครั้ง"
    " ขอบคุณครับ/ค่ะ"
)

# ==========================================
# 🔘 ปุ่มลิงค์นำทาง (Home และ Status)
# ==========================================
col_home, col_status = st.columns(2)
...
if not os.path.exists(DB_FILE):
  pd.DataFrame(columns=STANDARD_COLS).to_csv(
...
with st.form(key="student_form", clear_on_submit=True):
  student_id = st.text_input(
      "รหัสนักศึกษา:", placeholder="กรอกรหัสนักศึกษา 11 หลัก", max_chars=11
  )
  ...
  if student_id.strip():
    if not student_id.strip().isdigit():
      ...
      st.warning(
          f"ปัจจุบันกรอกไปแล้ว {len(student_id.strip())} หลัก (ต้องครบ 11"
          " หลัก)"
      )
      id_is_valid = False

  student_name = st.text_input(
      "ชื่อ - นามสกุล:", placeholder="กรอกชื่อและนามสกุล (เช่น นายสมชาย ดีใจ)"
  )
  ...
  if student_name.strip():
    if " " not in student_name.strip():
      st.warning(
          "กรุณากรอกทั้งชื่อและนามสกุล โดยเคาะเว้นวรรคตรงกลางด้วยครับ"
      )
      name_is_valid = False

  student_congenital_disease = st.text_input(
      "โรคประจำตัว:", placeholder="ถ้าไม่มีให้เว้นว่างไว้ หรือพิมพ์ -"
  )
  ...
  departments = [
      ...
      "คอมพิวเตอร์ธุรกิจ",
      "โลจิสติกส์",
      "อื่น ๆ (โปรดระบุรายละเอียดข้างล่าง)",
  ]
  selected_dept = st.selectbox("แผนกวิชา / สาขางาน:", departments)

  custom_dept = st.text_input(
      "ระบุแผนกวิชาเพิ่มเติม (กรณีเลือกอื่น ๆ):",
      placeholder="ระบุชื่อแผนกวิชาของคุณ",
  )

  if selected_dept.startswith("อื่น ๆ") and custom_dept.strip():
    final_dept = custom_dept.strip()
  elif custom_dept.strip():
    final_dept = f"{selected_dept} ({custom_dept.strip()})"
  else:
    final_dept = selected_dept

  symptoms = [
      "ปวดศีรษะ / เป็นไข้",
      "ปวดท้อง / ท้องเสีย / ท้องอืด",
      "ทำแผลอุบัติเหตุ / มีบาดแผล",
      "หน้ามืด / เป็นลม / คลื่นไส้",
      "อาการแพ้ / ผื่นคัน / แมลงสัตว์กัดต่อย",
      "กล้ามเนื้ออักเสบ / เคล็ดขัดยอก",
      "อื่น ๆ (โปรดระบุรายละเอียดข้างล่าง)",
  ]
  selected_symptom = st.selectbox("อาการป่วยเบื้องต้น:", symptoms)
  additional_detail = st.text_input(
      "รายละเอียดอาการเพิ่มเติม (ถ้ามี):",
      placeholder="ระบุรายละเอียดอาการป่วยเพิ่มเติม",
  )
  ...
  if submit_btn:
    if not student_id.strip() or not student_name.strip():
      st.error(
          "ไม่สามารถบันทึกได้:"
          " กรุณากรอกรหัสนักศึกษาและชื่อ-นามสกุลให้ครบถ้วนก่อนครับ"
      )
    elif selected_dept.startswith("อื่น ๆ") and not custom_dept.strip():
      st.error(
          "ไม่สามารถบันทึกได้:"
          " กรุณาระบุชื่อแผนกวิชาของคุณในช่องระบุเพิ่มเติมด้วยครับ"
      )
    elif not id_is_valid:
      st.error(
          "ไม่สามารถบันทึกได้: รหัสนักศึกษาไม่ถูกต้อง (ต้องเป็นตัวเลข 11"
          " หลัก)"
      )
      ...
      if not df.empty:
        student_records = df[
            df["รหัสนักศึกษา"].astype(str) == student_id.strip()
        ]
        if not student_records.empty:
          last_record = student_records.iloc[-1]
          try:
            last_time = datetime.strptime(
                last_record["วันที่-เวลา"], "%Y-%m-%d %H:%M:%S"
            )
      ...
      if is_duplicate:
        st.warning(
            "🛈 ระบบได้รับข้อมูลคิวของคุณเรียบร้อยแล้ว"
            " ไม่จำเป็นต้องกดส่งซ้ำซ้อน"
        )
      else:
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        disease_val = (
            student_congenital_disease.strip()
            if student_congenital_disease.strip()
            else "-"
        )
        ...
        new_record = {
            ...
            "status": "กำลังรอเจ้าหน้าที่คัดกรอง",
            "การจ่ายยา/หมายเหตุ": "-",
            "อาจารย์ผู้บันทึก": "-",
        }
        ...
        tg_msg = (
            f"[รายงานคิวผู้ป่วยใหม่ - RYTC Care]\nชื่อ:"
        ...
        st.success(
            f"บันทึกข้อมูลของ {student_name.strip()} สำเร็จ"
            " ระบบได้ส่งคิวไปยังคุณครูพยาบาลเรียบร้อยแล้ว"
        )
        st.info(
            " ท่านสามารถคลิกเลือกเมนู 'Status'"
            " ที่แถบเมนูด้านซ้ายมือหรือปุ่มด้านบนเพื่อดูสถานะคิวของท่านได้ทันที"
        )