...
# ==========================================
# ฟังก์ชันจัดการภาพพื้นหลังอัจฉริยะ (เสถียร 100%)
# ==========================================
    ...
    for path in possible_paths:
        if os.path.exists(path):
            try:
                ...
                # ปริ้นท์ error ล็อกไว้ดูใน Terminal เผื่อไฟล์ภาพมีปัญหา
...
with st.form(key="student_form", clear_on_submit=True):
    ...
    # 🔥 ย้าย Logic Validation ทั้งหมดมารวมกันเมื่อกด Submit
    if submit_btn:
        # ตรวจสอบค่าว่าง
        ...
        # ตรวจสอบความถูกต้องของ รหัสนักศึกษา
            ...
            st.error(f"ไม่สามารถบันทึกได้: รหัสนักศึกษาต้องครบ 11 หลักพอดี (ปัจจุบันกรอก {len(student_id.strip())} หลัก)")
            
        # ตรวจสอบความถูกต้องของ ชื่อ-นามสกุล
        elif " " not in student_name.strip():
            st.error("ไม่สามารถบันทึกได้: กรุณากรอกทั้งชื่อและนามสกุล โดยเคาะเว้นวรรคตรงกลางด้วยครับ")
            
        else:
            # ดำเนินการเมื่อข้อมูลผ่านการตรวจสอบทั้งหมด
