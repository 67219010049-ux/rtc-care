...
# ==========================================
# ฟังก์ชันจัดการภาพพื้นหลังอัจฉริยะ + ปรับแต่งสไตล์ (Responsive ปรับขนาดอัตโนมัติ)
# ==========================================
def set_perfect_background():
    ...
    if encoded_string:
        ...
        /* 2. ปรับแต่งช่องใส่ข้อความ (Input) */
        ...
        /* 🎨 ปรับคำแนะนำ 'Press Enter...' และคำอธิบายปุ่มฟอร์มบนหน้าจอคอมพิวเตอร์ (Desktop) */
        ...
        /* 🚫 พอกดส่งข้อมูลสำเร็จ (ช่องกรอกว่างเปล่า) -> ให้ซ่อนคำแนะนำทันที */
        div[data-testid="stTextInput"] input:placeholder-shown ~ [data-testid="stWidgetInstructions"],
        div[data-testid="stTextInput"] input:placeholder-shown ~ small,
        div[data-testid="stForm"] input:placeholder-shown ~ span[data-testid="stWidgetInstructions"] {{
        ...
        /* ปรับแต่งกล่อง Selectbox (Dropdown) */
        ...
        /* ปรับแต่งกรอบฟอร์มสี่เหลี่ยมด้านนอก */
        ...
        /* 3. ปรับแต่งปุ่มส่งข้อมูลในฟอร์ม */
        ...
        .responsive-h1 {{
            ...
            font-weight: bold; 
            margin-bottom: 0;
            font-size: calc(1.4rem + 1.0vw) !important;
        ...
        /* 📱 ปรับแต่งความกว้างและองค์ประกอบสำหรับหน้าจอมือถือ */
        @media (max-width: 768px) {{
            ...
            /* 🛠️ สั่งซ่อนคำแนะนำ "Press Enter" เฉพาะจุดที่ไม่ได้อยู่ภายในแท็ก button (เพื่อป้องกันปุ่มพัง) */
            div[data-testid="stForm"] div:not(button) > [data-testid="stWidgetInstructions"],
            div[data-testid="stForm"] div:not(button) > [class*="stWidgetInstructions"],
            .stTextInput div:nth-child(3),
            div.stFormSubmitButton + div,
            div.stFormSubmitButton > small {{
                display: none !important;
                visibility: hidden !important;
                height: 0px !important;
            ...
            /* 🔒 ล็อกสเปกพิเศษ: บังคับให้ปุ่มและ Element ด้านในปุ่มแสดงผลเป็นสีขาวตัวหนาเสมอ ไม่โดนคำสั่งซ่อนรบกวน */
            div.stFormSubmitButton > button,
            div.stFormSubmitButton > button * {{
                display: block !important;
                visibility: visible !important;
...
with st.form(key="student_form", clear_on_submit=True):
    ...
    # 1. กล่องรหัสนักศึกษา
    student_id = st.text_input("รหัสนักศึกษา:", placeholder="กรอกรหัสนักศึกษา", max_chars=11)
    ...
    # 2. กล่องชื่อ-นามสกุล
    student_name = st.text_input("ชื่อ - นามสกุล:", placeholder="กรอกชื่อและนามสกุล")
    ...
    if submit_btn:
        if not student_id.strip() or not student_name.strip():
            ...
            # ตรวจสอบและเปลี่ยนชื่อคีย์ให้ตรงกับหัวตาราง CSV เก่าป้องกัน error คอลัมน์สลับ
            final_record = {}
