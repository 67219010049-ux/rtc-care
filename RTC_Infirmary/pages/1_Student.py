...
# ==========================================
# ฟังก์ชันจัดการภาพพื้นหลัง (ปรับแต่งโครงสร้าง CSS ใหม่)
# ==========================================
def set_perfect_background():
    ...
    if encoded_string:
        ...
        bg_css = f"""
        <style>
        /* บังคับใส่ภาพพื้นหลังและ Overlay สีขาวจาง ๆ ลงไปในทุก Layer หลัก */
        html, body, [data-testid="stAppViewContainer"], [data-testid="stAppViewMain"], .stApp, .main, [data-testid="stMain"], [data-testid="stMainBlockContainer"] {{
        ...
        /* ปลดล็อกสีพื้นหลังของกล่องครอบเนื้อหาชั้นใน ให้โปร่งใสทะลุเห็นพื้นหลัง */
        [data-testid="stAppViewMain"], [data-testid="stMain"], [data-testid="stMainBlockContainer"] {{
        ...
        # หากระบบหาไฟล์ภาพไม่เจอ จะใช้สีพื้นหลังนวลตาสำรองไว้
        print("❌ [WARNING] หาไฟล์ชื่อ bg.jpg ไม่พบในระบบ! กำลังเปิดใช้พื้นหลังสีนวลสำรอง")
        st.markdown("<style>html, body, .stApp, [data-testid='stAppViewContainer'], [data-testid='stMainBlockContainer'] {background-color: #f4f7fc !important;}</style>", unsafe_allow_html=True)
...
with st.form(key="student_form", clear_on_submit=True):
    ...
    symptoms = [
        ...
        "ทำแงานอุบัติเหตุ / มีบาดแผล", "หน้ามืด / เป็นลม / คลื่นไส้", 
    ...
    # ==========================================
    # ระบบตรวจสอบเงื่อนไขหลังจากกด Submit
    # ==========================================
    if submit_btn:
