...
# ==========================================
# ฟังก์ชันจัดการภาพพื้นหลังอัจฉริยะ (แก้ไขจุดล็อก CSS)
# ==========================================
def set_perfect_background():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # รวม Path ยอดฮิตเพื่อให้เจอไฟล์แน่นอน ไม่ว่าจะรันด้วยโครงสร้างโฟลเดอร์แบบไหน
    possible_paths = [
    ...
    if encoded_string:
        bg_css = f"""
        <style>
        /* 🛠️ จุดแก้ไขหลัก: เจาะจงคลาสสำหรับคุม Background คลุมทุกเวอร์ชันของ Streamlit */
        ...
        /* 🛠️ ปรับแต่งกล่อง Selectbox ให้พื้นหลังเข้มและตัวหนังสือเป็น "สีขาว" */
        ...
        /* 🛠️ บังคับสไตล์ในหน้าจอมือถืออย่างเด็ดขาด */
        @media (max-width: 768px) {{
            .stApp, [data-testid="stAppViewContainer"], [data-testid="stApp"], .main {{
                ...
                background-size: cover !important;
                background-position: center !important;
                background-attachment: scroll !important; /* ป้องกันบั๊กเลื่อนบน iOS มือถือ */
            }}
        ...
        # แอบทำสีขาวนวลสำรองไว้ เผื่อลืมสร้างโฟลเดอร์แอปจะได้ไม่พัง
        st.markdown("<style>.stApp {background-color: #f4f7fc !important;}</style>", unsafe_allow_html=True)
...
with st.form(key="student_form", clear_on_submit=True):
    ...
    if student_name.strip():
        if " " not in student_name.strip():
            st.warning("⚠️ กรุณากรอกทั้งชื่อและนามสกุล โดยเคาะเว้นวรรคตรงกลางด้วยครับ")
            name_is_valid = False
