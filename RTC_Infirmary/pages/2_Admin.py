...
if st.session_state.admin_logged_in:
    ...
    if not df.empty:
        ...
        m1, m2 = st.columns(2)
        m1.metric("🏥 จำนวนนักเรียนที่ใช้บริการรวม", f"{len(df)} เคส")
        m2.metric("⏳ เคสที่ยังค้างอยู่ในระบบ (รอตรวจ)", f"{len(df[df['สถานะการรักษา'] == '⏳ กำลังรอเจ้าหน้าที่ตรวจ'])} เคส")
    ...
    if df.empty:
        ...
        waiting_list = df[df["สถานะการรักษา"] == "⏳ กำลังรอเจ้าหน้าที่ตรวจ"]["ชื่อ-นามสกุล"].tolist()
        
        if not waiting_list:
            st.success("🎉 ตรวจรักษาเสร็จสิ้นครบทุกเคสเรียบร้อยแล้ว!")
        else:
            selected_student = st.selectbox("เลือกชื่อนักเรียนที่ต้องการวินิจฉัย:", waiting_list)
            student_info = df[df["ชื่อ-นามสกุล"] == selected_student].iloc[-1]
            st.warning(f"📋 เคสของ: {selected_student} | แผนก: {student_info['แผนกวิชา']} | อาการ: {student_info['อาการเบื้องต้น']}")
            ...
            if update_btn:
                ...
                if selected_qty > current_stock_qty:
                    ...
                    idx = df[df["ชื่อ-นามสกุล"] == selected_student].index[-1]
