...
if st.session_state.admin_logged_in:
    ...
    # ==========================================
    # 🧹 ฟังก์ชันโหลดข้อมูล + ล้างประวัติยาที่ยกเลิกออกจากระบบถาวร
    # ==========================================
    def load_data():
        if os.path.exists(DB_FILE): 
            load_df = pd.read_csv(DB_FILE)
            
            # 🔥 [แก้บัคตรงนี้] ระบุคำคีย์เวิร์ดของยาที่พี่ต้องการสั่งให้ "ลบทิ้งถาวร" ออกจากตารางและไฟล์ CSV 
            # (ระบบจะสแกนหาคำเหล่านี้ทั้งในช่อง อาการ และช่องรายละเอียด แล้วลบแถวนั้นทิ้งทันที)
            EXCLUDE_KEYWORDS = ["ยาขับเลือด", "ยาสตรี", "ยานอนหลับ", "ยาบำรุงเลือด"]
            
            for keyword in EXCLUDE_KEYWORDS:
                # 1. ลบจากคอลัมน์การจ่ายยา
                if "การจ่ายยา/หมายเหตุ" in load_df.columns:
                    load_df = load_df[~load_df["การจ่ายยา/หมายเหตุ"].astype(str).str.contains(keyword, na=False)]
                # 2. ลบออกจากคอลัมน์อาการเบื้องต้นด้วย (เพื่อความชัวร์ 100%)
                if "อาการเบื้องต้น" in load_df.columns:
                    load_df = load_df[~load_df["อาการเบื้องต้น"].astype(str).str.contains(keyword, na=False)]
            
            # ลบคอลัมน์ขยะอื่น ๆ ที่อาจตกค้าง
            ...
            if "สถานะการรักษา" in load_df.columns:
                ...
                load_df.loc[load_df["開設สถานะการรักษา"].astype(str) == "None", "สถานะการรักษา"] = "⏳ กำลังรอเจ้าหน้าที่คัดกรอง"
            else:
    ...
    # กรองคลังหลังบ้านไม่ให้คำเหล่านี้หลงเหลืออยู่เช่นกัน
    EXCLUDE_STOCK_MEDS = ["ยาขับเลือด", "ยาสตรี", "ยานอนหลับ", "ยาบำรุงเลือด"]
