...
# ==========================================
# 🎨 ฟังก์ชันจัดการภาพพื้นหลัง [แก้ไขระบบดักจับตำแหน่งไฟล์ให้ค้นหาเจอทุกที่]
# ==========================================
def set_perfect_background():
    ...
    if encoded_string:
        ...
        /* 2. ล็อกสีข้อความเฉพาะในส่วนเนื้อหาหลัก */
        ...
        /* 🔴 [แก้ไขจุดที่เป็นปัญหาสำเร็จ!] บังคับสีปุ่ม "ออกจากระบบ" (Logout) ให้เป็นสีแดงสดร้อยเปอร์เซ็นต์ ไม่ให้โดนสไตล์อื่นบดบัง */
        div.stButton > button {{
        ...
        /* บังคับสีตัวอักษรคำว่า "ออกจากระบบ" ด้านในปุ่มให้เป็นสีขาวหนา เด่นชัด เจาะลึกถึงโครงสร้างชั้นใน */
        ...
        /* เอฟเฟกต์เมื่อเมาส์ชี้ไปที่ปุ่มออกจากระบบ (จะเปลี่ยนเป็นสีแดงเข้มขึ้นเล็กน้อยเพื่อให้มีมิติ) */
        ...
        /* 🟢 ตกแต่งปุ่มดาวน์โหลดรายงาน (Download Button) ให้เป็นสีเขียวสว่าง ชัดเจน ไม่ดำมืด */
        ...
        /* ปุ่มสีฟ้าพิเศษเฉพาะส่วนบันทึกฟอร์มวินิจฉัยโรค */
...
# เรียกใช้ฟังก์ชันเซ็ตอัพพื้นหลังแบบอัจฉริยะ
...
# 1. ระบบความจำการล็อกอิน
if "admin_logged_in" not in st.session_state:
    st.session_state.admin_logged_in = False
if "current_user" not in st.session_state:
    st.session_state.current_user = ""

# 2. ฟอร์มล็อกอินเข้าใช้งาน
...
# 3. หากเข้าสู่ระบบผ่านเรียบร้อยแล้ว
if st.session_state.admin_logged_in:
    ...
    with col_logout:
        # 🔴 [ปุ่มออกจากระบบสีแดงสว่างสดใส ตัวอักษรขาวหนา คมชัดสะดุดตา]
    ...
    if not df.empty:
        ...
        # ปรับการนับเคสค้างให้ตรวจจับแบบยืดหยุ่นด้วย "รอ"
        waiting_count = len(df[df['สถานะการรักษา'].astype(str).str.contains('รอ', na=False)])
        m2.metric("⏳ เคสที่ยังค้างอยู่ในระบบ (รอตรวจ)", f"{waiting_count} เคส")
    ...
    if df.empty:
        ...
        # 🛠️ [จุดที่แก้ไขแก้ไข] เปลี่ยนมาค้นหาคำว่า "รอ" แทนการเช็กตัวอักษรเป๊ะๆ ป้องกันปัญหาเรื่องช่องว่างหรืออีโมจิไม่ตรงกัน
        waiting_df = df[df["สถานะการรักษา"].astype(str).str.contains("รอ", na=False)]
        
        # นำ "รหัสนักศึกษา" มาพ่วงท้ายชื่อใน Selectbox เพื่อป้องกันปัญหาชื่อซ้ำแล้วระบบพัง
        if waiting_df.empty:
            waiting_options = []
        else:
        ...
        if not waiting_options:
            ...
            # ตัดสตริงดึงชื่อจริงแยกกลับออกมาเพื่อค้นหาใน DataFrame ดั้งเดิม
    ...
    # ==========================================
    # ⚙️ แผงบริหารและจัดการคลังยา
    # ==========================================
    st.write("---")
    st.markdown("### ⚙️ แผงบริหารและจัดการคลังยา (Inventory Management Control)")
    
    stock_tab1, stock_tab2 = st.tabs(["➕ เติมยา / แก้ไขยอดคงเหลือด่วน", "✨ ลงทะเบียนเพิ่มยาชนิดใหม่เข้าคลัง"])
    
    with stock_tab1:
        st.markdown("##### 🔄 ระบบเติมยาเข้าคลัง หรือ พิมพ์ยอดแก้ไขสต็อกที่ผิดพลาด")
        add_col1, add_col2, add_col3, add_col4 = st.columns([2, 1.2, 1.2, 1], gap="medium")
    ...
    with stock_tab2:
        ...
        new_col1, new_col2, new_col3, new_col4 = st.columns([2, 1, 1, 1], gap="medium")
        with new_col1:
            new_med_name = st.text_input("✍️ ชื่อยา/เวชภัณฑ์ใหม่ที่ต้องการเพิ่ม:", placeholder="เช่น ยาแก้ไอขับเสมหะ, ผ้าก๊อซพันแผล")
        with new_col2:
            new_med_unit = st.text_input("📦 หน่วยนับ:", placeholder="เช่น เม็ด, ขวด, ม้วน, ชิ้น", value="เม็ด")
        with new_col3:
    ...
    # 📥 ปุ่มดาวน์โหลดรายงานสีเขียวเด่นสะดุดตา ตัวหนังสือขาวคมชัด
    if not df.empty:
        st.write("---")
