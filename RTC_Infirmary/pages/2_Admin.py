...
if st.session_state.admin_logged_in:
    ...
    # ==========================================
    # 🧹 ฟังก์ชันโหลดข้อมูล + เคลียร์คอลัมน์ซ้ำซ้อนอัตโนมัติ
    # ==========================================
    def load_data():
        if os.path.exists(DB_FILE): 
            load_df = pd.read_csv(DB_FILE)
            
            # 🔥 [แก้บัคสำคัญ] ลบคอลัมน์ขยะหรือคอลัมน์ที่ชื่อเพี้ยน ซ้ำซ้อน ออกไปให้หมด
            bad_cols = [c for c in load_df.columns if "สถานะการรักษา." in c or "暗" in c or c == "Unnamed: 0"]
            if bad_cols:
                load_df = load_df.drop(columns=bad_cols)
            
            # หากในอดีตคอลัมน์ "สถานะการรักษา" หลักมีค่าเป็น None หรือว่างเปล่า ให้แปลงกลับเป็น "⏳ กำลังรอเจ้าหน้าที่คัดกรอง"
            if "สถานะการรักษา" in load_df.columns:
                load_df["สถานะการรักษา"] = load_df["สถานะการรักษา"].fillna("⏳ กำลังรอเจ้าหน้าที่คัดกรอง")
                # สำหรับแถวไหนที่เป็นคำว่า "None" (ตัวหนังสือ) ให้แก้คืนเป็น "⏳ กำลังรอเจ้าหน้าที่คัดกรอง"
            ...
            # เรียงลำดับคอลัมน์มาตรฐานให้ถูกต้อง ไม่สลับตำแหน่ง
    ...
    # ==========================================
    # 📊 รายงานสถิติภาพรวม (เรียง 2 กล่องในบรรทัดเดียวตามสั่ง)
    # ==========================================
    if not df.empty:
        ...
        with m2:
            # ตรวจเช็กผู้ที่ยังมีสถานะ "รอ" อยู่ในข้อความอย่างแม่นยำ
    ...
    if df.empty:
        ...
        # คัดกรองเฉพาะเคสที่ยัง "รอ" ตรวจอย่างถูกต้องแม่นยำ
        ...
        if not waiting_options:
            ...
            # 🔓 ปุ่มจ่ายยาและรายชื่อจะยอมแสดงผลขึ้นมาตามปกติแล้วครับ
