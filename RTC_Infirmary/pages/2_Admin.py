...
# ==========================================
# 🚀 รหัส Telegram สำเร็จรูปของอาจารย์
# ==========================================
...
# ==========================================
# 🎨 ฟังก์ชันจัดการภาพพื้นหลัง (สไตล์เดิมเป๊ะๆ)
# ==========================================
def set_perfect_background():
    possible_paths = [
        "RTC_Infirmary/images/bg.png",  
        "images/bg.png",                
        "bg.png"                        
    ]
...
# ==========================================
# 🏛️ หัวข้อหน้าเว็บแบบ Responsive
# ==========================================
...
if not st.session_state.admin_logged_in:
    ...
    with col_m:
        ...
        login_btn = st.button("🚀 เข้าสู่ระบบ (Login)", type="primary", use_container_width=True)
...
if st.session_state.admin_logged_in:
    ...
    # ==========================================
    # 🧹 ฟังก์ชันโหลดข้อมูล + กรองยี่ห้อยาที่ถูกลบทิ้งไปแล้วออกจากระบบ
    # ==========================================
    def load_data():
        if os.path.exists(DB_FILE): 
            load_df = pd.read_csv(DB_FILE)
            
            # 🛠️ [ตั้งค่าระบุชื่อที่พี่อยากตัดทิ้งที่นี่ได้เลยครับ] 
            EXCLUDE_MEDICINES = ["ยาขับเลือด", "ยาสตรี", "ยานอนหลับ"]
            
            # ทำการคัดกรองแถวที่มีคำว่ายาเหล่านั้นออกจากข้อมูลดิบของตารางและไฟล์ CSV ทันที
            has_changed = False
            ...
            # ถ้าระบบตรวจพบประวัติขยะหลงเหลืออยู่ จะทำการบันทึกเซฟทับลง CSV ให้สะอาดทันทีอัตโนมัติ
            if has_changed:
                load_df.to_csv(DB_FILE, index=False, encoding="utf-8-sig")
            
            bad_cols = [c for c in load_df.columns if "暗" in c or c == "Unnamed: 0"]
            # ยอมให้คอลัมน์ที่มีจุด (ผลจากการแก้ไข) ถูกเอาออกอย่างปลอดภัย
            ...
            if "สถานะการรักษา" in load_df.columns:
                load_df["暗สถานะการรักษา"] = load_df["สถานะการรักษา"].fillna("⏳ กำลังรอเจ้าหน้าที่คัดกรอง")
                # คืนค่ากลับเป็นชื่อมาตรฐาน
                load_df["สถานะการรักษา"] = load_df["暗สถานะการรักษา"]
                load_df = load_df.drop(columns=["暗สถานะการรักษา"])
                load_df.loc[load_df["สถานะการรักษา"].astype(str) == "None", "สถานะการรักษา"] = "⏳ กำลังรอเจ้าหน้าที่คัดกรอง"
            else:
            ...
            standard_cols = ["วันที่-เวลา", "รหัสนักศึกษา", "ชื่อ-นามสกุล", "แผนกวิชา", "อาการเบื้องต้น", "สถานะการรักษา", "การจ่ายยา/หมายเหตุ", "อาจารย์ผู้บันทึก"]
            for col in standard_cols:
                if col not in load_df.columns:
                    load_df[col] = "-"
            return load_df[standard_cols]
            
        return pd.DataFrame(columns=["วันที่-เวลา", "รหัสนักศึกษา", "ชื่อ-นามสกุล", "แผนกวิชา", "อาการเบื้องต้น", "สถานะการรักษา", "การจ่ายยา/หมายเหตุ", "อาจารย์ผู้บันทึก"])
    ...
    # ดักกรองรายชื่อยาขยะในไฟล์คลังยาหลังบ้านด้วยเช่นกัน
    EXCLUDE_STOCK_MEDS = ["ยาขับเลือด", "ยาสตรี", "ยานอนหลับ"]
    df_stock = df_stock[~df_stock["ชื่อยา/เวชภัณฑ์"].isin(EXCLUDE_STOCK_MEDS)]
    ...
    if not df.empty:
        ...
        with m2:
            waiting_count = len(df[df['สถานะการรักษา'].astype(str).str.contains('รอ|⏳', na=False)])
        ...
        with tab1:
            ...
            fig_bar = px.bar(dept_counts, x="แผนกวิชา", y="จำนวนนักเรียน (คน)", color="แผนกวิชา", 
        ...
        with tab2:
            try:
                ...
                st.info("💡 ข้อมูลวันที่ยังไม่สมบูรณ์สำหรับการพล็อตเส้นเวลา")
    ...
    # 🔍 ระบบค้นหาประวัติ
    # ==========================================
    st.markdown("### 🔍 ตรวจสอบประวัติการรับยาย้อนหลังรายบุคคล")
    ...
    if search_id.strip():
        ...
        if not student_history.empty:
            ...
            found_user_name = student_history.iloc[-1]["ชื่อ-นามсков"] if "ชื่อ-นามсков" in student_history.columns else student_history.iloc[-1]["ชื่อ-นามสกุล"]
            
            with search_col2:
                if total_records >= 4:
                    st.warning(f"⚠️ นักศึกษาคนนี้มารับบริการค่อนข้างบ่อยผิดปกติ!")
                st.metric(label="📊 จำนวนครั้งที่เข้าใช้บริการห้องพยาบาลรวม", value=f"{total_records} ครั้ง")
            
            st.markdown(f"##### 🗓️ ตารางประวัติของ: <span style='color:#007bff; font-weight:bold;'>{found_user_name}</span> [รหัส: {search_id}]", unsafe_allow_html=True)
            st.dataframe(student_history[["วันที่-เวลา", "อาการเบื้องต้น", "สถานะการรักษา", "การจ่ายยา/หมายเหตุ", "อาจารย์ผู้บันทึก"]], use_container_width=True)
        else:
            with search_col2:
                st.info("🆕 ไม่พบประวัติเก่าในระบบ (นักศึกษาคนนี้เพิ่งเคยมาเป็นครั้งแรก)")
    
    st.write("---")

    if df.empty:
        st.warning("📭 ปัจจุบันยังไม่มีข้อมูลนักเรียนมาลงชื่อเข้ารับบริการ")
    else:
        st.subheader("📋 รายชื่อนักเรียนคิวเข้ารับบริการทั้งหมด")
        st.dataframe(df, use_container_width=True)
        
        # ==========================================
        # 🚨 [เพิ่มใหม่] แผงปุ่มสำหรับให้อาจารย์ลบประวัตินักเรียนรายบุคคลออกจากตาราง
        # ==========================================
        with st.expander("🗑️ แผงควบคุมเจ้าหน้าที่: ลบประวัตินักเรียนออกจากฐานข้อมูล"):
            ...
            # สร้างรายการคิวให้น่าเชื่อถือระบุแถวได้ชัดเจน: วันที่ - ชื่อ (รหัส) - อาการ
            ...
            # แกะข้อมูลออกมาเทียบแถว
            del_time = selected_student_to_delete.split(" | ")[0]
            del_name = selected_student_to_delete.split(" | ")[1].split(" (")[0]
            
            admin_delete_confirm = st.button("🔥 ยืนยันลบประวัตินักเรียนรายนี้ออกถาวร", use_container_width=True, key="admin_delete_student_btn")
            if admin_delete_confirm:
                # ทำการกรองเก็บทุกคนไว้ ยกเว้นคนที่มีเงื่อนไข "วันที่" และ "ชื่อ" ตรงกับตัวที่เลือก
                df = df[(df["วันที่-เวลา"] != del_time) | (df["ชื่อ-นามสกุล"] != del_name)]
                save_data(df)
                st.success(f"🗑️ ลบประวัติข้อมูลคิวของ {del_name} ออกจากระบบ CSV เรียบร้อยแล้ว!")
                st.rerun()

        st.write("---")
        st.subheader("⚡ บันทึกผลการคัดกรองและรักษา")
        
        waiting_df = df[df["สถานะการรักษา"].astype(str).str.contains("รอ|⏳", na=False)]
        
        if waiting_df.empty:
            waiting_options = []
        else:
            waiting_options = [f"{row['暗ชื่อ-นามสกุล'] if '暗ชื่อ-นามสกุล' in row else row['ชื่อ-นามสกุล']} ({row['รหัสนักศึกษา']})" for idx, row in waiting_df.iterrows()]
        ...
        if not waiting_options:
            ...
            student_info = waiting_df[waiting_df["暗ชื่อ-นามสกุล" if "暗ชื่อ-นามสกุล" in waiting_df.columns else "ชื่อ-นามสกุล"] == selected_student_name].iloc[-1]
            
            st.warning(f"📋 เคสของ: {selected_student_name} | แผนก: {student_info['แผนกวิชา']} | อาการ: {student_info['อาการเบื้องต้น']}")
            
            treatment_status = st.radio(
                "💉 สั่งการรักษาทางแพทย์:",
                ["💊 จ่ายยาและให้กลับไปเรียนต่อ", "🛏️ ให้นอนพักฟื้นที่ห้องพยาบาล", "🚑 ส่งต่อโรงพยาบาลภายนอก (เคสฉุกเฉิน)"]
            )
            ...
            treatment_detail = st.text_input("✍️ ระบุรายละเอียดตัวยาที่จ่าย / อาการเพิ่มเติม:")
    ...
    for s_idx, s_row in df_stock.iterrows():
        with stock_cols[s_idx]:
            if s_row["คงเหลือ"] <= 20:
                st.error(f"🚨 **{s_row['ชื่อยา/เวชภัณฑ์']}** \n\n ใกล้หมด! เหลือ: {s_row['คงเหลือ']} {s_row['หน่วย']}")
            else:
                st.success(f"🔹 **{s_row['ชื่อยา/เวชภัณฑ์']}** \n\n คงเหลือ: {s_row['คงเหลือ']} {s_row['หน่วย']}")

    st.write("---")
    st.markdown("### ⚙️ แผงบริหารและจัดการคลังยา (Inventory Management Control)")
    ...
    with stock_tab1:
        ...
        with add_col2:
            manage_mode = st.radio("เลือกการทำงาน:", ["➕ เติมเพิ่มเข้าคลัง", "🛠️ พิมพ์แก้ตัวเลขโดยตรง"], key="manage_mode_radio")
        with add_col3:
            ...
            if manage_mode == "➕ เติมเพิ่มเข้าคลัง":
                ...
                current_val = int(df_stock.at[r_idx_check, "คงเหลือ"])
                input_qty = st.number_input(f"ยอดคงเหลือที่ถูกต้อง ({unit_label}):", min_value=0, max_value=5000, value=current_val, key="rewrite_num")
        with add_col4:
    ...
    with stock_tab2:
        ...
        with new_col1:
            new_med_name = st.text_input("✍️ ชื่อยา/เวชภัณฑ์ใหม่ที่ต้องการเพิ่ม:", placeholder="เช่น ยาแก้ไอขับเสมหะ, ผ้าก๊อซพันแผล")
        with new_col2:
        ...
        if add_new_med_btn:
            if not new_med_name.strip():
                ...
                new_row = {"ชื่อยา/เวชภัณฑ์": new_med_name.strip(), "คงเหลือ": new_med_qty, "หน่วย": new_med_unit.strip()}
    ...
    with stock_tab3:
        st.markdown("##### 🗑️ ระบบลบรายชื่อยาและกวาดล้างประวัติขยะถาวร")
        ...
        st.markdown("<p style='color:#dc3545; font-size:13px;'>⚠️ หมายเหตุ: การกดปุ่มนี้จะลบตัวยาออกจากคลัง และกวาดล้างประวัติเก่าทั้งหมดที่มีชื่อยานี้อยู่ทิ้งทันที เพื่อไม่ให้มีคำนี้ติดไปในไฟล์รายงาน CSV ตอนส่งออกครับ</p>", unsafe_allow_html=True)
        ...
        if execute_delete_btn:
            ...
            st.success(f"🗑️ ลบ '{delete_med_target}' และกวาดล้างข้อมูลขยะออกจากไฟล์ CSV เรียบร้อยแล้วครับพี่!")
            st.rerun()
