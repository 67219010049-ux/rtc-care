...
if st.session_state.admin_logged_in:
    ...
    def load_data():
        if os.path.exists(DB_FILE): 
            ...
            bad_cols = [c for c in load_df.columns if "สถานะการรักษา." in c or "暗" in c or c == "Unnamed: 0"]
            if bad_cols:
                load_df = load_df.drop(columns=bad_cols)
            
            if "สถานะการรักษา" in load_df.columns:
                load_df["สถานะการรักษา"] = load_df["สถานะการรักษา"].fillna("⏳ กำลังรอเจ้าหน้าที่คัดกรอง")
                load_df.loc[load_df["สถานะการรักษา"].astype(str) == "None", "สถานะการรักษา"] = "⏳ กำลังรอเจ้าหน้าที่คัดกรอง"
            else:
                load_df["開設สถานะการรักษา"] = "⏳ กำลังรอเจ้าหน้าที่คัดกรอง"
    ...
    # ⚙️ ทำการเพิ่มแท็บที่ 3 สำหรับการสั่งลบยาและประวัติผ่านหน้าเว็บโดยตรง ชัวร์เด็ดขาด 100%
    ...
    with stock_tab3:
        ...
        execute_delete_btn = st.button("🔥 ยืนยันลบยาและล้างประวัติคำนี้ออกจากระบบเด็ดขาด", use_container_width=True, key="execute_delete_btn")
        
        if execute_delete_btn:
            # 1. ลบยาออกจากคลังหลังบ้าน
            df_stock = df_stock[df_stock["ชื่อยา/เวชภัณฑ์"] != delete_med_target]
            df_stock.to_csv(STOCK_FILE, index=False, encoding="utf-8-sig")
            
            # 2. เปิดไฟล์ประวัติขึ้นมาสแกนลบแถวที่มีชื่อยานี้ออกถาวร
            if os.path.exists(DB_FILE):
